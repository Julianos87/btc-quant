...
- **Secrets** : aucune clé API, jeton ou identifiant commis dans le dépôt. Les clés d'exchange (`ccxt_broker.py`), `TELEGRAM_BOT_TOKEN` / `TELEGRAM_CHAT_ID` (`notify.py`) et `DASHBOARD_TOKEN` (`dashboard/app.py`) sont systématiquement lus via l'environnement, jamais codés en dur. `.gitignore` exclut `.env`, `state/`, `backups/`, `data/`.
...
- **Logique financière** (le point le plus sensible de ce projet) : dimensionnement des positions, coupe-circuits (drawdown/perte journalière), persistance atomique de l'état, idempotence des ordres et comptabilisation du fill réellement exécuté sont couverts par des tests dédiés. Le stop suiveur ne peut jamais être desserré (`BacktestEngine`/`LiveRunner`, testé explicitement). L'ancien adaptateur carry Binance non branché a été supprimé afin de ne plus suggérer qu'un chemin live double-jambe existe.
