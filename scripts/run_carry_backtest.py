...
def main() -> None:
    ...
    print(f"Funding : {len(funding)} paiements, {funding.index[0].date()} -> {funding.index[-1].date()}\n")

    print(f"{'entrée>':>8} {'sortie<':>8} {'lissage':>8} | {'CAGR':>7} {'Sharpe':>7} {'max DD':>7} "
          f"{'exposition':>11} {'cycles':>7}")
    for enter in (0.03, 0.05, 0.08):
        for exit_ in (0.0, 0.02):
            for smooth in (3, 7, 14):
                r = backtest_carry(funding, leverage=args.leverage,
                                   enter_ann=enter, exit_ann=exit_, smooth_days=smooth)
                print(f"{enter:>7.0%} {exit_:>8.0%} {smooth:>6}j  | {r['cagr']:>+7.1%} "
