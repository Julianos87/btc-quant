...
def main() -> None:
    ...
    print(f"Spot BTC/USDT : min qty {spot['min_qty']} BTC, min notionnel {spot['min_notional']} $\n")
    ...
    for cap in args.capital:
        ...
        c_ok = (carry_qty >= (perp["min_qty"] or 0) and carry_qty >= (spot["min_qty"] or 0)
                and carry_notional >= (perp["min_notional"] or 0))

        print(f"── capital {cap:,.0f} $ ──")
        print(f"  trend/slot ({slot_equity:,.0f} $) : qty {qty:.5f} BTC, notionnel {notional:,.0f} $ "
              f"→ {'✓ passe' if trend_ok else '✗ SOUS LE MINIMUM'}")
        print(f"  carry ({cap*(1-TARGET_TREND):,.0f} $ à {CARRY_LEVERAGE:.0f}x) : "
