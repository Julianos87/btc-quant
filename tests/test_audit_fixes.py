...
def make_df(n: int = 14, price: float = 100.0) -> pd.DataFrame:
    ...
    o = np.full(n, price)
    return pd.DataFrame(
        {"open": o, "high": o + 1.5, "low": o - 1.5, "close": o + 0.5,
...
def risk_simple() -> RiskConfig:
    return RiskConfig(initial_capital=10_000.0, risk_per_trade=0.01,
                      max_position_pct=0.95, vol_target_annual=None,
                      max_drawdown_halt=0.99, daily_loss_limit=None)
...
def test_funding_column_overrides_constant():
    ...
    res = BacktestEngine(fee_rate=0.0, slippage_bps=0.0, risk=risk_simple(),
                         funding_rate_8h=99.0).run(strat, df)
    qty = 100.0 / 10.0
    ...
    assert abs(got - expected_funding) < 1e-6, f"funding réel non appliqué : {got} vs {expected_funding}"
...
def test_funding_column_absent_falls_back_to_constant():
    ...
    res = BacktestEngine(fee_rate=0.0, slippage_bps=0.0, risk=risk_simple(),
                         funding_rate_8h=rate_8h).run(strat, df)
    qty = 100.0 / 10.0
...
def test_fill_from_order_no_amount_fallback():
    ...
    order2 = {"average": 100.0, "filled": 2.0, "amount": 5.0,
              "fees": [], "fee": {"cost": 0.12}}
    fill2 = CcxtBroker._fill_from_order(object.__new__(CcxtBroker), order2, 99.0)
    assert fill2.qty == 2.0 and abs(fill2.fee - 0.12) < 1e-12


def test_runner_state_atomic_roundtrip():
    """État sauvegardé atomiquement (pas de .tmp restant) et entry_fee persisté."""
    import tempfile
    ...
    slot.position = Position(
        entry_time=pd.Timestamp("2026-01-01", tz="UTC"), entry_price=100.0,
        qty=2.0, stop_price=90.0, direction=1, bars_held=3, best_close=105.0,
    )
    runner = LiveRunner([slot], PaperBroker(), risk_simple(),
...
if __name__ == "__main__":
    ...
    test_runner_state_atomic_roundtrip()
