...
def make_df(opens: list[float], lows: list[float] | None = None,
...
def test_short_win():
    ...
    strat = MockStrategy(enter_bar=5, direction=-1, stop_dist=10.0, exit_after=4)
    res = BacktestEngine(fee_rate=FEE, slippage_bps=SLIP_BPS,
                         risk=risk_simple(), allow_short=True).run(strat, df)
...
def test_short_blocked_when_not_allowed():
    ...
    res = BacktestEngine(fee_rate=FEE, slippage_bps=SLIP_BPS,
                         risk=risk_simple(), allow_short=False).run(strat, df)
...
def test_position_size_caps():
    cfg = RiskConfig(initial_capital=10_000, risk_per_trade=0.01, max_position_pct=0.95,
                     vol_target_annual=0.40, max_drawdown_halt=0.99, daily_loss_limit=None)
